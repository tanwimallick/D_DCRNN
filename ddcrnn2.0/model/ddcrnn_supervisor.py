...
from model.ddcrnn_model import DCRNNModel
...
class DCRNNSupervisor(object):
    ...
    def __init__(self, data, kwargs):
        ...
        grads = tf.gradients(ys=self._train_loss, xs=tvars)
    ...
    def _build_sparse_matrix(L):
        ...
        return tf.compat.v1.SparseTensorValue(indices, L.data, L.shape)
    ...
    def run_epoch_generator(self, sess, model, data_generator, return_output=False, training=False, writer=None):
        ...
        for idx, (x, y) in enumerate(data_generator):
            ...
            #print('adj_mx.isnull().values.any()', np.isnan(adj_mx).any())
            adj_mx[np.isnan(adj_mx)] = 0.01
    ...
    def predict(self, sess, data_test, **kwargs):
        global_step = sess.run(tf.compat.v1.train.get_or_create_global_step())
