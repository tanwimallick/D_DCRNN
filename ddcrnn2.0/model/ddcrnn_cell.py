...
class DCGRUCell(RNNCell):
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        x = inputs_and_state
        x0 = tf.transpose(a=x, perm=[1, 2, 0])  # (num_nodes, total_arg_size, batch_size)
        ...
        with tf.compat.v1.variable_scope(scope):
            if self._max_diffusion_step == 0:
                ...
                x1 = tf.sparse.sparse_dense_matmul(self._adj_mx, x0)
                x = self._concat(x, x1)

                for k in range(2, self._max_diffusion_step + 1):
                    #x2 = 2 * tf.sparse.sparse_dense_matmul(support, x1) - x0
                    x2 = 2 * tf.sparse.sparse_dense_matmul(self._adj_mx, x1) - x0
                    x = self._concat(x, x2)
                    x1, x0 = x2, x1
            ...
            x = tf.transpose(a=x, perm=[3, 1, 2, 0])  # (batch_size, num_nodes, input_size, order)
